...
-> because the diffrent datasets measure different things
...
data_amp_fast <- data_amp_filtered %>% 
...
data_demographics_tidied <- data_demographics_trimmed %>% 
  filter(item == "age") %>% 
  filter(response != "old")
...
data_selfreport_tidied <- data_selfreport_filtered %>% 
...
# write examples here
#i used something else. pros: less coding. cons: u can't follow every step of the code
...
data_amp_inverted <- data_amp_filtered %>% 
...
```{r}
#could i just use summary for the first task? (not sure if that's what's asked)
 data_selfreport_tidied %>% 
  summary()

#data_selfreport_new <- data_selfreport_tidied 

#data_selfreport_new %>% 
#  count(unique_id, name = "n_trials") %>% 
 # count(n_trials, name = "n_participants")
  
# i am lost here it doesnt seem to count like it used to ... so icouldnt exclude participants with incomplete data
