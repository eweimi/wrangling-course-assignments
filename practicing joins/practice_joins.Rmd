...
data_missing_ids <- data_missing_ids %>%
  ...
  select(unique_id) %>% #we only need unique id for the answer
  distinct(unique_id) # to sort out duplicates

  
```

Be careful with full joins because you can end with loads of duplicates
