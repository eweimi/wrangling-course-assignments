...
plot_1 <- 
  ...
  scale_shape_manual(values = c("female" = 16, 
                     ...
                     labels = c("female" = "Female",
                                "male" = "Male")) +
  # display specific x and y coordinates without dropping data points (nb using `limits` drops data points, coord_cartesian does not) plotted nur gewisse daten aber berücksichtigt alle Daten (keine Verfälschung der Daten)
...
ggplot(data = data_after_exclusions,
       aes(x = mean_self_report)) +
  # more intelligent choices for the binwidth and boundary
  # aber achtung density plot sind immer ausgeglättet und nicht GENAU die Daten
  #hier sind color, adjust etc im geom und nicht im aes
