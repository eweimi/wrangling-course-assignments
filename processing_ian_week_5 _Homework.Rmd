...
- Exclude them
```{r}
data_selfreport_p3 <- data_selfreport_tidied2 %>% 
  count(unique_id, name = "num_trials") %>% 
  filter(num_trials == 3)
 
```
Besser mit group by und sumamrize machen weil, das ist eigentlich das gleiche wie count aber droppt nicht alle anderen spalten aus data¨
(Denn muessmer nacher nümme joine)

- check your exclusions worked. FRAGE!!! warum kommt beim n jetzt 1 und nicht 3???? --> muesch wieder joine
```{r}
